...
const colors = ["#5569FF", "#FF1943", "#FFA319", "#57CA22", "#9333ea", "#06b6d4", "#374151"]
...
const RejectionReasonsChart: React.FC = () => {
  ...
  // Process data to show API data + defaults up to MAX_ITEMS
  let displayData: RejectionReasonCount[]

  if (rawData.length === 0) {
    // No API data - show all defaults with 0 count
    displayData = defaultReasons.slice(0, MAX_ITEMS).map((reason) => ({
      reason,
      count: 0,
      percent: 0,
    }))
  } else if (rawData.length >= MAX_ITEMS) {
    // API has 6 or more items - show only API data (first 6)
    displayData = rawData.slice(0, MAX_ITEMS)
  } else {
    // API has some data but less than 6 - combine API data with defaults
    const apiReasons = rawData.map((item) => item.reason.toLowerCase())
    const remainingDefaults = defaultReasons
      .filter((reason) => !apiReasons.includes(reason.toLowerCase()))
      .slice(0, MAX_ITEMS - rawData.length)
      .map((reason) => ({
        reason,
        count: 0,
        percent: 0,
    ...
    displayData = [...rawData, ...remainingDefaults]
  }
  ...
  return (
    ...
    >
      <CardHeader
        ...
        action={<Chip label={`Total: ${totalCount}`} size="small" sx={{ fontWeight: 700 }} />}
      />
          ...
          {displayData.map((item, i) => {
            ...
            return (
              <Grid item xs={12} sm={6} md={4} key={i}>
                <Box mb={2}>
                  <Typography variant="body2" fontWeight={600} color="text.primary" mb={1} noWrap title={item.reason}>
                    {item.reason}
                  </Typography>
