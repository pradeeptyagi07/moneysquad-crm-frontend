...
export const createPartner = createAsyncThunk(
  ...
  async (form: PartnerFormData, { rejectWithValue }) => {
    try {
      ...
      // Personal Info
      formData.append("personalInfo.gender", form.gender.toLowerCase()); // enforce lowercase
      formData.append("personalInfo.dateOfBirth", form.dateOfBirth || "");
      formData.append("personalInfo.employmentType", form.employmentType);
      formData.append(
      ...
      if (form.cancelledCheque)
        formData.append("cancelledCheque", form.cancelledCheque);
      ...
      return rejectWithValue(
        error.response?.data?.message || "Partner creation failed"
      );
    }
  }
);
